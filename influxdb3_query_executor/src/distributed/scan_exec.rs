...
//!
//! Optimizations for JOIN queries:
//! 1. Batch prefetching when part of a JOIN operation
//! 2. Result caching for repeated scans
//! 3. JOIN-aware query building
...
///
/// JOIN Optimizations:
/// - Increased channel buffer size for JOIN operations
/// - Batch prefetching hint for better throughput
/// - Statistics caching for cardinality estimation
#[derive(Debug)]
pub struct RemoteTableScanExec {
    ...
    /// Hint: is this scan part of a JOIN operation?
    /// This enables optimizations like increased buffering
    is_join_input: bool,
}
...
impl RemoteTableScanExec {
    ...
    pub fn new(
        ...
        Self::new_with_join_hint(
            ...
            limit,
            rpc_client,
            false, // default: not a JOIN input
        )
    }

    /// Create a new remote table scan with JOIN optimization hint
    pub fn new_with_join_hint(
        node_id: NodeId,
        database: String,
        table_name: String,
        schema: SchemaRef,
    ...
    /// Get optimal buffer size based on whether this is a JOIN input
    fn get_buffer_size(&self) -> usize {
        if self.is_join_input {
            // For JOIN operations, use larger buffer to improve throughput
            // This allows more concurrent data fetching and reduces back-pressure
            100
        } else {
            // Default buffer size for regular scans
            10
        }
    }
...
impl ExecutionPlan for RemoteTableScanExec {
    ...
    fn execute(
        ...
        // Create a channel with dynamic buffer size
        // For JOIN operations, use larger buffer to improve throughput
